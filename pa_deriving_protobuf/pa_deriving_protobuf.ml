...
module PM = ParamMap(struct value arg_ctyp_f loc pty = <:ctyp< $pty$ -> Pa_ppx_deriving_protobuf_runtime.Protobuf.Encoder.t -> unit >> ; end) ;
...
value prim_encoder loc attrmod fname =
  ...
  <:expr< let open Pa_ppx_deriving_protobuf_runtime.Encode in
    $wrapped$ >>
;
...
value sig_item_fun0 arg td =
  ...
  let argfmttys = List.map (fun pty -> <:ctyp< $pty$ -> Pa_ppx_deriving_protobuf_runtime.Protobuf.Encoder.t -> unit >>) paramtys in  
  let ty = <:ctyp< $lid:tyname$ >> in
  let toftype = Ctyp.arrows_list loc argfmttys <:ctyp< $(Ctyp.applist ty paramtys)$ -> Pa_ppx_deriving_protobuf_runtime.Protobuf.Encoder.t -> unit >> in
...
module PM = ParamMap(struct value arg_ctyp_f loc pty = <:ctyp< Pa_ppx_deriving_protobuf_runtime.Protobuf.Decoder.t -> $pty$ >> ; end) ;
...
let i1_from_protobuf_manually decoder =
  ...
  let rec derec v0 = match Protobuf.Decoder.key decoder with
    | Some (1, kind) ->
      let v0 = update_v0 v0 
      (let open Pa_ppx_deriving_protobuf_runtime.Decode in
...
let i1_from_protobuf_manually = (*e1*) fun decoder ->
  ...
  (*e4*)let rec derec (<tuple-of-vars>) = match Protobuf.Decoder.key decoder with
    ...
    | Some(<key_i>, Protobuf.Bytes) ->
      (*e5b*)let open Pa_ppx_deriving_protobuf_runtime.Decode in
...
value demarshal_to_tuple loc arg am_kind_fmt_vars =
  ...
  let branch5s = List.map (fun (am, kind, fmt, v) ->
    ...
    let e5b =
      <:expr< let open Pa_ppx_deriving_protobuf_runtime.Decode in
...
value of_expression arg ~{attrmod} param_map ty0 =
  ...
  <:expr< let open Pa_ppx_deriving_protobuf_runtime.Decode in
          decode0 { kind = Protobuf.Varint ; convertf = Pa_ppx_deriving_protobuf_runtime.forget1 $lid:bare_fname$ ; decodef = Protobuf.Decoder.varint } ~{msg=msg} >>)
...
| <:ctyp:< $longid:li$ . $lid:lid$ >> when attrmod.bare ->
  ...
  (attrmod, <:patt< Protobuf.Varint >>,
  <:expr< let open Pa_ppx_deriving_protobuf_runtime.Decode in
          decode0 { kind = Protobuf.Varint ; convertf = Pa_ppx_deriving_protobuf_runtime.forget1 $bare_f$ ; decodef = Protobuf.Decoder.varint } ~{msg=msg} >>)
...
| <:ctyp:< [= $list:l$ ] >> as ty0 when attrmod.bare ->
  ...
  (attrmod, <:patt< Protobuf.Varint >>,
  <:expr< let open Pa_ppx_deriving_protobuf_runtime.Decode in
          decode0 { kind = Protobuf.Varint ; convertf = Pa_ppx_deriving_protobuf_runtime.forget1 $f$ ; decodef = Protobuf.Decoder.varint } ~{msg=msg} >>)
...
| [%unmatched_vala] -> failwith "pa_deriving_protobuf.of_expression"
  ...
  | <:ctyp:< [ $list:_$ ] >> | <:ctyp:< [= $list:_$ ] >> ->
    <:expr< let open Pa_ppx_deriving_protobuf_runtime.Decode in $fmt$ >>
  | _ ->
    let loc = loc_of_ctyp ty0 in
